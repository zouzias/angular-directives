...
angular.module('d3Components')
  .controller('MainCtrl', function ($scope) {
    ...
    $scope.reload = function () {
      ...
      $scope.histogram = {};
      $scope.histogram.buckets = [];
      var x = 0.0;
      for ( var i = 0; i < 30 ; i++){
        var randomInterval = 0.1 + Math.random();
        $scope.histogram.buckets.push({'x' : x , 'interval' : randomInterval , 'count' : randInteger(10)});
        x += randomInterval;
      }

      $scope.histogram.max = d3.max($scope.histogram.buckets, function(d){return d.x + d.interval});
      $scope.histogram.min = d3.min($scope.histogram.buckets, function(d){return d.x});
